                ...
                <div className="modal-body">
                    ...
                    <div className="user-stats-grid">
                        ...
                        <div className="user-stat-card">
                            ...
                            <div className="stat-content">
                                <span className="stat-value">₹{Number(user.totalSpent ?? 0).toLocaleString()}</span>
                        ...
                        <div className="user-stat-card">
                            ...
                            <div className="stat-content">
                                <span className="stat-value">{user.createdAt ? new Date(user.createdAt).toLocaleDateString() : '—'}</span>
                        ...
                        <div className="user-stat-card">
                            ...
                            <div className="stat-content">
                                <span className="stat-value">{user.lastActive ?? (user.updatedAt ? new Date(user.updatedAt).toLocaleDateString() : '—')}</span>
...
const UserManagement = ({ users, orders, isLoading }) => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        // Sorting (safe for missing orders/totalSpent/lastActive from API)
        result.sort((a, b) => {
            switch (userSortBy) {
                case 'name': return (a.name || '').localeCompare(b.name || '');
                case 'orders': return Number(b.orders ?? 0) - Number(a.orders ?? 0);
                case 'spent': return Number(b.totalSpent ?? 0) - Number(a.totalSpent ?? 0);
                case 'recent': return new Date(b.lastActive || b.updatedAt || 0) - new Date(a.lastActive || a.updatedAt || 0);
                default: return 0;
            }
        });
            ...
            {isLoading ? (
                ...
                <div className="data-table-container">
                    <table className="data-table">
                        ...
                        <tbody>
                            {filteredUsers.map(u => (
                                <tr key={u._id || u.id} onClick={() => setSelectedUser(u)}>
                                    <td>
                                    ...
                                    <td>
                                        <span className={`profile-badge ${(u.profileType || 'Regular').toLowerCase()}`}>
                                            {u.profileType || 'Regular'}
                                        </span>
                                    </td>
                                    <td>{u.orders ?? '—'}</td>
                                    <td>₹{Number(u.totalSpent ?? 0).toLocaleString()}</td>
                                    <td>{u.lastActive ?? (u.updatedAt ? new Date(u.updatedAt).toLocaleDateString() : u.createdAt ? new Date(u.createdAt).toLocaleDateString() : '—')}</td>
                                    <td>
